...
import { getAlerts, getFollowUps } from '../../lib/dashboard';
import { MenuIcon, SearchIcon, BellIcon, ClockIcon, ChevronDownIcon, LogOutIcon } from './Icons';
...
const Topbar = ({ title, onMenuClick }: TopbarProps) => {
  ...
  const [openMenu, setOpenMenu] = useState<'alerts' | 'followups' | 'user' | null>(null);
  ...
  const followUpsRef = useClickOutside(() => setOpenMenu((m) => (m === 'followups' ? null : m)));
  const userRef = useClickOutside(() => setOpenMenu((m) => (m === 'user' ? null : m)));

  const toggle = (menu: 'alerts' | 'followups' | 'user') =>
  ...
  return (
    <header className="shell-topbar">
      ...
      <div className="shell-topbar-right">
        ...
        <div className="shell-dropdown-wrap" ref={followUpsRef} style={{ position: 'relative' }}>
          <button className="shell-icon-btn" onClick={() => toggle('followups')} aria-label="Follow-ups due">
            <ClockIcon />
            {!!followUps.data?.length && <span className="shell-icon-badge amber">{followUps.data.length > 9 ? '9+' : followUps.data.length}</span>}
          </button>
          {openMenu === 'followups' && (
            <div className="shell-dropdown">
              <div className="shell-dropdown-header">Follow-ups Due</div>
              <div className="shell-dropdown-list">
                {followUps.loading && <div className="shell-dropdown-empty">Loading…</div>}
                {!followUps.loading && !followUps.data?.length && <div className="shell-dropdown-empty">Nothing due</div>}
                {followUps.data?.slice(0, 8).map((f) => (
                  <div className="shell-dropdown-item" key={f.consultationId}>
                    <span className={`shell-dropdown-dot ${f.isOverdue ? 'red' : 'amber'}`} />
                    <span>
                      {f.patientName} — {f.isOverdue ? 'overdue' : 'due'} {new Date(f.followUpDate).toLocaleDateString()}
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div ref={userRef} style={{ position: 'relative' }}>
          <button className="shell-user" onClick={() => toggle('user')}>
            ...
            <div className="shell-user-text">
              <span className="shell-user-name">{user ? `Dr. ${user.username}` : 'Doctor'}</span>
              <span className="shell-user-role">Doctor</span>
            </div>
            <ChevronDownIcon />
          </button>
          {openMenu === 'user' && (
