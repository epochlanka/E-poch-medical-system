...
import { createMedicine, updateMedicine, BASE_UNITS, PACK_UNITS, DOSAGE_FORMS, defaultBaseUnitForForm } from '../../lib/medicines';
import type { MedicineCatalogRow, CatalogMeta, StockBatchInput } from '../../lib/medicines';
import { getSuppliers } from '../../lib/operations';
import type { Supplier } from '../../lib/operations';
...
// ---- Edit / View: a quick single-page form for master-data fields only — packaging/pricing for
...
const EditOrViewForm = ({ mode, medicine, meta, onClose, onSaved }: Props & { medicine: MedicineCatalogRow }) => {
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card" style={{ maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <h3 className="modal-title">{mode === 'edit' ? 'Edit Medicine' : 'Medicine Details'}</h3>
          <button type="button" className="pat-icon-btn" onClick={onClose} aria-label="Close">
            <XIcon />
          </button>
        </div>
        <p className="modal-subtitle">Product details only — stock and pricing live on this medicine's batches.</p>
        ...
        <form onSubmit={submit}>
          <div className="modal-grid">
            ...
            <div className="modal-field">
              ...
              <select value={form.base_unit} onChange={set('base_unit')} disabled={readOnly} required>
                {BASE_UNITS.map((u) => (
            ...
            <div className="modal-field">
              ...
              <select value={form.default_pack_unit} onChange={set('default_pack_unit')} disabled={readOnly}>
            ...
            <div className="modal-field">
              ...
              <input type="number" min={0} value={form.default_pack_size} onChange={set('default_pack_size')} disabled={readOnly} />
            </div>
            <div className="modal-field">
            ...
            <div className="modal-field">
              ...
              <input type="number" min={0} step="0.01" value={form.default_selling_price} onChange={set('default_selling_price')} disabled={readOnly} />
            </div>
            <div className="modal-field">
...
// ---- Add: the 5-step wizard (Section 6) --------------------------------------------------
...
const AddWizard = ({ meta, onClose, onSaved }: Props) => {
  const [step, setStep] = useState(0);
  const [form, setForm] = useState<AddFormState>(emptyAddForm);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const { data: suppliers } = useApiData<Supplier[]>(() => getSuppliers(), []);
  ...
  const onFormChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setForm((f) => ({ ...f, form: dosageForm, base_unit: f.base_unit === defaultBaseUnitForForm(f.form) || !f.form ? defaultBaseUnitForForm(dosageForm) : f.base_unit }));
  };
  ...
  const goNext = () => {
    ...
    setError(null);
    // Skip the Initial Stock / Pricing steps entirely if the pharmacist chose not to add opening stock.
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card" style={{ maxWidth: 640 }} onClick={(e) => e.stopPropagation()}>
        ...
        {/* Step 2: Packaging & Unit Configuration */}
        {step === 1 && (
          <div>
            <p className="wiz-section-title">Packaging & Unit Configuration</p>
            <p className="wiz-section-help">The base unit is what stock and pricing are tracked in. Packaging is a default only — each batch can still be received differently.</p>
            <div className="modal-grid">
              <div className="modal-field">
                <label>Base Unit *</label>
        ...
        {/* Step 3: Initial Stock & Batch Details */}
        {step === 2 && (
          <div>
        ...
        {step === 4 && (
          <div>
            ...
            <div className="wiz-review-row">
              ...
              <span>
                {form.default_pack_unit ? `${form.default_pack_size || '?'} ${form.base_unit} per ${form.default_pack_unit}` : 'Not set'}
              </span>
            </div>
            <div className="wiz-review-row">
...
const MedicineFormModal = (props: Props) => {
  if (props.mode === 'add') return <AddWizard {...props} />;
  if (!props.medicine) return null;
  return <EditOrViewForm {...props} medicine={props.medicine} />;
};
