...
function MedicineChecklistRow({ item, batches, choice, expanded, externalMarked, onChoose, onChange, onClear, onExpand, onExternal }:
  ...
  return <article className={`gd-medicine ${choice?.picked || externalMarked ? 'chosen' : ''}`}>
    ...
    <p className="gd-need-line"><strong>{balance} {item.medicine.base_unit} needed now</strong>{item.dispensed_qty > 0 && <> · {item.dispensed_qty} already recorded</>}{item.external_qty > 0 && <> · {item.external_qty} recorded as obtained outside</>}</p>
...
function DispensingWorkspace({ prescriptionId }: { prescriptionId: number }) {
  ...
  return <main className="gd-page">
    ...
    {!needsReload && <section className="gd-panel"><details open={finished}><summary className="gd-history-summary">Recorded medicines & printing ({recorded.length} batch records)</summary><p className="gd-muted">Clinic quantities already saved. Each batch is shown separately.</p>{recorded.length ? <div className="gd-list">{recorded.map(event => <div className="gd-record" key={event.dispense_id}><div><strong>{event.batch.medicine.name} · {event.qty} {event.batch.medicine.base_unit}</strong><small>Batch {event.batch.batch_no} · Expires {date(event.batch.expiry_date)} · Recorded {date(event.dispensed_at)}</small>{event.instructions && <small>{event.instructions}</small>}</div></div>)}</div> : <p className="gd-muted">No clinic medicine recorded yet.</p>}
    ...
    {review && <dialog ref={reviewDialog} className="gd-modal" aria-labelledby="gd-review-title" onCancel={e => { e.preventDefault(); if (!busy) { setReview(null); reviewButton.current?.focus(); } }}>
      ...
      <div className="gd-review-list">{detail.items.map(item => {
        ...
        return <div key={item.rx_item_id}>
          ...
          <span>{item.dispensed_at ? 'Already recorded' : !line ? 'Not giving now' : line.batch_id ? `Give ${line.qty} ${item.medicine.base_unit} · Batch ${batch?.batchNo} · Expires ${batch ? date(batch.expiryDate) : '—'}` : 'Confirmed obtained outside clinic — no stock used'}</span>
          <small>{!item.dispensed_at && !line ? 'Not recorded now — still needs action' : !item.dispensed_at && left > 0 ? `${left} ${item.medicine.base_unit} still waiting` : 'No quantity waiting after this'}</small>
        </div>;
      })}</div>
