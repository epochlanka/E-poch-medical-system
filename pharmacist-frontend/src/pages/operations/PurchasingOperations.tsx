...
import type { Grn, PurchaseOrder, Supplier } from '../../lib/operations';
...
type ReceiptLine = { qty: string; batch: string; expiry: string };
...
export default function PurchasingOperations({ mode }: { mode: Mode }) {
  ...
  const saveReceipt = () => {
    ...
    const items = selectedOrder.items.filter(i => Number(receipt[i.po_item_id]?.qty) > 0).map(i => ({ po_item_id: i.po_item_id, qty_received: Number(receipt[i.po_item_id].qty), batch_no: receipt[i.po_item_id].batch.trim(), expiry_date: receipt[i.po_item_id].expiry }));
    if (!items.length || items.some(i => !Number.isInteger(i.qty_received) || !i.batch_no || !i.expiry_date || new Date(i.expiry_date) <= new Date())) { setError('For each received medicine, enter a whole-number quantity, batch number, and future expiry date.'); return; }
  ...
  const updateReceipt = (id: number, key: keyof ReceiptLine, value: string) => setReceipt(v => ({ ...v, [id]: { qty: v[id]?.qty ?? '', batch: v[id]?.batch ?? '', expiry: v[id]?.expiry ?? '', [key]: value } }));
