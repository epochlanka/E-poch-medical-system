...
export default function InventoryOperations({ mode }: { mode: Mode }) {
  ...
  const submitAdjustment = () => {
    ...
    void run(() => adjustStock(selectedId, amount, reason.trim()), 'Stock change saved with your reason.').then(ok => { if (ok) { setDelta(''); setReason(''); } });
  };
  const submitCount = () => {
