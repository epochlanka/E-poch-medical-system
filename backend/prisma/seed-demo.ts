...
async function main() {
  ...
  const amoxicillin = await prisma.medicine.upsert({
    ...
    create: {
      ...
      base_unit: 'Capsule',
      default_pack_unit: 'Strip',
      default_pack_size: 10,
      default_selling_price: 45,
      reorder_level: 50,
      barcode: '8901030002',
    },
  });
  ...
  const batch1 = await prisma.batch.upsert({
    ...
    create: {
      ...
      purchase_date: new Date('2026-07-05'),
  ...
  const batch2 = await prisma.batch.upsert({
    ...
    create: {
      ...
      received_unit: 'Capsule',
      received_qty: 90,
      units_per_pack: 1,
      qty_base_total: 90,
      qty_on_hand: 80,
      purchase_price_per_pack: 30,
  ...
  // Priced slightly higher than batch2 — a later restock at a different cost, demonstrating that
  // profit is tracked per batch rather than assuming one flat price for the whole medicine.
  const batch3 = await prisma.batch.upsert({
    ...
    create: {
      ...
      purchase_date: new Date('2026-07-22'),
      ...
      qty_base_total: 60,
      qty_on_hand: 60,
      purchase_price_per_pack: 32,
      cost_per_base_unit: 32,
      selling_price_per_base_unit: 48,
      supplier_id: supplier.supplier_id,
    },
  });
  await prisma.stockLedger.upsert({
  ...
  const invoice1 = await prisma.invoice.upsert({
    ...
    create: {
      ...
      patient_id: patient.patient_id,
      consultation_id: consultation.consultation_id,
      subtotal: CONSULTATION_FEE + 20 * paracetamol.default_selling_price + 10 * amoxicillin.default_selling_price,
      discount_total: 0,
      total_amount: CONSULTATION_FEE + 20 * paracetamol.default_selling_price + 10 * amoxicillin.default_selling_price,
      paid_amount: CONSULTATION_FEE + 20 * paracetamol.default_selling_price + 10 * amoxicillin.default_selling_price,
      payment_status: 'Paid',
      created_by: receptionist.user_id,
    },
  });
  ...
  await prisma.invoiceItem.upsert({
    ...
    create: {
      ...
      qty: 20,
      unit_price: paracetamol.default_selling_price,
      line_total: 20 * paracetamol.default_selling_price,
  ...
  await prisma.invoiceItem.upsert({
    ...
    create: {
      ...
      qty: 10,
      unit_price: amoxicillin.default_selling_price,
      line_total: 10 * amoxicillin.default_selling_price,
  ...
  const outstandingInvoice = await prisma.invoice.upsert({
    ...
    create: {
      ...
      subtotal: CONSULTATION_FEE + 5 * cetirizine.default_selling_price,
      total_amount: CONSULTATION_FEE + 5 * cetirizine.default_selling_price,
      paid_amount: 300,
  ...
  await prisma.invoiceItem.upsert({
    ...
    create: {
      ...
      qty: 5,
      unit_price: cetirizine.default_selling_price,
      line_total: 5 * cetirizine.default_selling_price,
  ...
  const masterData: { type: string; value: string; sort_order: number }[] = [
    ...
    { type: 'PaymentMethod', value: 'Bank Transfer', sort_order: 4 },
    { type: 'PaymentMethod', value: 'Other', sort_order: 5 },
  ...
  const labCatalog: {
  ...
  }[] = [
    {
      ...
      parameters: [
        ...
        { parameter_name: 'WBC Count', unit: 'Ã—10â¹/L', reference_range: '4-11' },
        { parameter_name: 'Platelets', unit: 'Ã—10â¹/L', reference_range: '150-450' },
        { parameter_name: 'RBC Count', unit: 'Ã—10Â¹Â²/L', reference_range: '4.5-5.5' },
  ...
  // The rows above are inserted with explicit ids (medicine_id: 1, ...). On PostgreSQL that does
  // NOT advance the SERIAL sequences, so the next auto-generated id collides with a seeded row
  // ("Unique constraint failed on medicine_id"). Move every sequence past its table's max id.
