...
const MergeFamilyModal = ({ family, onClose, onMerged }: MergeFamilyModalProps) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    listFamilies({ search, status: 'active', limit: 8 })
      .then((res) => {
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="pat-search" style={{ marginBottom: 12 }}>
          <SearchIcon />
          <input placeholder="Search the surviving family by name…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>

        <div style={{ maxHeight: 220, overflowY: 'auto', marginBottom: 4 }}>
          {loading && <p className="modal-subtitle">Searching…</p>}
          {!loading && search.trim().length >= 2 && results.length === 0 && <p className="modal-subtitle">No other active families found.</p>}
          {!loading && search.trim().length < 2 && <p className="modal-subtitle">Type at least 2 characters to search.</p>}
          {!loading &&
            results.map((f) => (
              <div
                key={f.family_id}
              ...
              >
                ...
                <span className="pat-muted">{f._count.patients} members</span>
              </div>
            ))}
        </div>
